...
describe('RunicDisplay', () => {
  it('renders SVG when runic line numbers are provided', () => {
    ...
    const { container } = render(<RunicDisplay runicLineNumbers={lines} />);
    
    expect(container.querySelector('svg')).not.toBeNull();
  });

  it('displays message when no lines are provided', () => {
    const { getByText } = render(<RunicDisplay runicLineNumbers={[]} />);
  ...
  it('has aria-label for accessibility', () => {
    ...
    const svgContainer = container.querySelector('[aria-label="Runic representation of entered number"]');
