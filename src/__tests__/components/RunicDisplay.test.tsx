...
describe('RunicDisplay', () => {
  it('renders SVG when runic line numbers are provided', () => {
    ...
    const { container } = render(<RunicDisplay runicObject={{runicLineNumbers: lines, inputValue: 10}} />);
    
    expect(container.querySelector('svg')).not.toBeNull();
  });

  it('displays message when no lines are provided', () => {
    const { getByText } = render(<RunicDisplay runicObject={{runicLineNumbers: [], inputValue: 0}} />);
